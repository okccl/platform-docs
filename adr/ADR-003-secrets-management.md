...
ローカル環境（Phase 2）ではESOのFakeプロバイダーで動作確認し、
Phase 10でSOPS×Ageによる暗号化管理を追加した。
...
**検証中の課題:**
- ArgoCDとSOPS×Ageの復号連携フロー（Age秘密鍵をどのように
  ArgoCDに渡すか）は現在検証中。確定次第このADRを更新する
